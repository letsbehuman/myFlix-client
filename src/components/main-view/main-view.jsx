...
function setItem(key, item) {
  const itemComoString = JSON.stringify(item);
...
class MainView extends React.Component {
  ...
  handleFav = (movieId, action) => {
    ...
    const url = `https://my-movie-db22.herokuapp.com/users/${Username}/${movieId}`;
    ...
    if (token !== null && Username !== null) {
      ...
      if (action === 'add') {
        ...
        movieService.removeFavoriteMovie(
          { Username, movieId },
          (res) => {
            alert(`Movie removed from ${Username} Favorite movies`);
            window.open(`/users/${Username}`, '_self');
          },
          (error) => this.errorCallback(error, 'removeFav Error ')
        );
        // axios
        //   .delete(url, {
        //     headers: { Authorization: `Bearer ${token}` },
...
class MovieService {
  constructor(token) {
    this.token = token;
    this.baseUrl = 'https://my-movie-db22.herokuapp.com/users';
    this.header = {
  ...
  addFavoriteMovie(payload, successCallback, errorCallback) {
    ...
    axios.post(url, {}, this.header).then(successCallback).catch(errorCallback);
  }

  removeFavoriteMovie(payload, successCallback, errorCallback) {
    const url = `${this.baseUrl}/${Username}/${movieId}`;
    axios.delete(url, this.header).then(successCallback).catch(errorCallback);
  }
}
